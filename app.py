from __future__ import annotations  # <-- fix forward refs for type hints
...
def create_cosinor_inputs(
    ids, group, replicate, time_series, time, group1_label, group2_label
):
    return f"""
    CosinorAnalysis(
    ids={ids},
        ...
        for i, id_curr in enumerate(study_list):
            ...
            for j in range(n_reps):
                ...
                x_processed, y_processed = self.get_trend(
                    x_fit, y_fit, method=method, window=window
                )
                if method != "none":
                    ax.plot(
                        x_processed,
            ...
            ax.set_title(f"ID: {study_list[i]} (n={n_reps}) - {group_label}")
...
def cosine_model_damped(x, amplitude, damp, acrophase, period, mesor):
    return (
        amplitude * np.exp(-damp * x) * np.cos(2 * np.pi * (x - acrophase) / period)
        + mesor
    )
...
class CosinorAnalysis(LiveCellDataset):
    ...
    def fit_cosinor_free_period(
        ...
        params_free, _ = curve_fit(
            cosine_model_free_period, x, y, p0=p0_free, bounds=bounds
        )
        amp_fit, acro_fit, period_fit, mesor_fit = params_free
        yhat_free = cosine_model_free_period(
            x, amp_fit, acro_fit, period_fit, mesor_fit
        )
        ...
        y_test_acro = cosine_model_free_period(
            t_test_acro, amp_fit, acro_fit, period_fit, mesor_fit
        )
        amplitude = abs(amp_fit)
        acrophase = t_test_acro[int(np.argmax(y_test_acro))]
        mesor = mesor_fit
        ...
        y_test = cosine_model_free_period(
            t_test, amp_fit, acro_fit, period_fit, mesor_fit
        )
    ...
    def fit_cosinor_damped(
        ...
        params_damped, _ = curve_fit(
            cosine_model_damped, x, y, p0=p0_damped, bounds=bounds
        )
        amp_fit, damp_fit, acro_fit, period_fit, mesor_fit = params_damped
        yhat_damped = cosine_model_damped(
            x, amp_fit, damp_fit, acro_fit, period_fit, mesor_fit
        )
        ...
        y_test_acro = cosine_model_damped(
            t_test_acro, amp_fit, damp_fit, acro_fit, period_fit, mesor_fit
        )
        amplitude = abs(amp_fit)
        acrophase = t_test_acro[int(np.argmax(y_test_acro))]
        mesor = mesor_fit
        ...
        y_test = cosine_model_damped(
            t_test, amp_fit, damp_fit, acro_fit, period_fit, mesor_fit
        )
        ...
        for i, id_curr in enumerate(study_list):
            ...
            for j in range(n_reps):
                ...
                results, t_test, model_predictions_cosine = self.get_cosinor_fits(
                    x_processed, y_detrended, method=cosinor_model
                )
...
class OmicsDataset:
    ...
    def _check_columns(self) -> Self:
        missing = [
            c
            for c in (self.columns_cond1 + self.columns_cond2)
...
class M2(BaseModel):
    ...
    k: int = 5
    formula: str = (
        "y ~ is_alpha:constant + is_beta:constant + is_beta:cos_wt + is_beta:sin_wt -1"
    )
...
class DifferentialRhythmicity:
    ...
    def rhythmic_genes_expressed_both(self, progress=gr.Progress()) -> pd.DataFrame:
        ...
        for gene, row in progress.tqdm(
            ...
            design: pd.DataFrame = build_design(
                alpha_vec, beta_vec, self.t_cond1, self.t_cond2
            )
    ...
    def rhythmic_genes_expressed_cond1(self) -> pd.DataFrame:
        ...
        for gene, row in tqdm(
            ...
            results: list[BaseModelOneCondition] = [
                m.fit(design) for m in MODELS_ONE_CONDITION
            ]
            bics: np.ndarray = np.array([r.bic for r in results])
            weights: np.ndarray = akaike_weights_from_bics(bics)
            if np.max(weights) < self.BIC_cutoff:
            ...
            rows.append(
                {
                    ...
                    # "weight": weights[pick],
                    **{
                        f"w_model{model}": weights[i - 1]
                        for i, model in enumerate([1, 3])
                    },
    ...
    def extract_all_circadian_params(self) -> pd.DataFrame:
        ...
        df_pre_export = self.df.merge(
            results_total, left_on="Genes", right_on="gene", how="right"
        )
        column_list: list = [
            "Genes",
            *self.columns_cond1,
...
class OmicsHeatmap:
    ...
    def timepoint_means(
        self, df: pd.DataFrame, columns: list[str], times: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def plot_heatmap(self, cmap: str = "bwr") -> None:
        ...
        mean_cond1: np.ndarray = self.timepoint_means(
            df, self.columns_cond1, self.t_cond1
        )
        mean_cond2: np.ndarray = self.timepoint_means(
            df, self.columns_cond2, self.t_cond2
        )
    ...
    with gr.Tabs() as tabs:
        with gr.Tab("Live cell", id=0):
            ...
            def process_csv(fpath):
                ...
                shape_info = (
                    f"Loaded shape: {df_data.shape} | "
                    f"participants: {len(participant_id)} | time points: {len(time)}"
                )
            ...
            group_choice = gr.Radio(
                ["group1", "group2"], value="group1", label="Select group to analyse"
            )
            m_slider = gr.Slider(
                1, 10, value=5, step=1, label="Number of columns for plot"
            )
            ...
            def make_plot(
                ...
                if method == "moving_average":
                    ...
                    fig, pdf_path = ds.plot_group_data(
                        which_group, method=method, m=int(m), plot_style=plot_style
                    )
                return fig, pdf_path
        ...
        with gr.Tab("Omics", id=1):
            ...
            gr.Examples(
                examples=[omics_example], inputs=omics_file, label="Example input"
            )
            ...
            override_time = gr.Checkbox(
                label="Override time vectors manually?", value=False
            )
            ...
            scatter_btn = gr.Button(
                "Generate replicate scatterplot", variant="secondary"
            )
            ...
            def build_omics_inputs(
                df, cols_a, cols_b, use_manual_time, t_a_text, t_b_text
            ):
                if df is None:
                    return "# Upload or select an example file first."

                t_a = _build_time_vec(
                    len(cols_a), t_a_text if use_manual_time else None
                )
                t_b = _build_time_vec(
                    len(cols_b), t_b_text if use_manual_time else None
                )
        ...
        """
            ...
            with gr.Row():
                cond1_label_tb = gr.Textbox(
                    label="Condition 1 label", value="Alpha cells"
                )
                cond2_label_tb = gr.Textbox(
                    label="Condition 2 label", value="Beta cells"
                )
                mean_min_num = gr.Number(
                    label="mean_min (for is_expressed)", value=0, precision=0
                )

            compute_dr_btn = gr.Button(
                "Compute differential rhythmicity & heatmap", variant="primary"
            )
            ...
            params_preview = gr.Dataframe(
                label="Rhythmic parameters (preview)", interactive=False
            )
            params_download = gr.File(label="Download rhythmic parameters (CSV)")

            def run_dr_and_heatmap(
                df,
                cols_a,
                cols_b,
                use_manual_time,
